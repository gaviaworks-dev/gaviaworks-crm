...
| `GV.proje.bakimPaketleri(kod)` · `GV.proje.bakimBagla(proje, paket)` · `GV.proje.bakimAc(proje, cfg)` | Kapanışın son adımı (REVİZE 08): müşterinin paketlerini listeler · var olan pakete bağlar (**başka müşterinin** ya da **başka projeye bağlı** paketi reddeder) · yeni paket açar. Bağ **pakette** durur (`DB.supportPackages[].proje`), projede ayna alan doğmaz. Yeni pakette kota sözleşmesi korunur: `kullanilan + kalan = aylikSaat × ay` | Proje ile paket arasında **hiçbir yönde bağ yoktu** ve veriden türetilemiyordu; tarih yakınlığı bağ değildir (L-13). Bağı kullanıcı kapanış anında kurar. Aylık saat ve bedel **sorulur** — paket fiyat katalogu bu depoda yok, uydurulmaz |
| `GV.destek.acik(t)` · `.kapali(t)` · `.kapaliDurumlar()` · `.cozumTarihi(t)` | Destek talebi durum ekseninin tek tanımı. `kapaliDurumlar()` **veriden** okur (`DB.ticketClosedStatuses`) çünkü `shell.js` aynı listeyi kullanır ve `domain.js`'ten önce yüklenir. `cozumTarihi` **alan değildir**, `acilis + cozumSuresi`den türetilir (L-08) | "Açık talep" tanımı **on iki** yerde elle yazılıydı (defter dokuz diyordu — L-28'in örüntüsü); sözlük yeniden adlandırılınca on ikisi birden **sessizce sıfır** sayardı |
...
| `GV.yenileme.uzat(tur, kod)` · `.onizleme(tur, kod)` · `.isaretle(tur, kod)` | Sözleşme ve hizmet paketi yenilemesinin **tek** mutasyon noktası. `tur ∈ {'sozlesme','paket'}` ve alan politikası türe göre yazılı: sözleşmede yalnız `bitis` (+ `yenilemeTarihi`) kayar, imza dönemi başlangıcı geçmiştir; pakette `baslangic` ve `bitis` birlikte kayar, böylece ay sayısı ve `kullanilan + kalan = aylikSaat × ay` kota sözleşmesi korunur. İkisi de aktivite yazar | Aynı iş iki ekranda ayrı yazılıydı ve **farklı davranıyordu**: sözleşme detayı aktivite kaydı düşüyordu, bakım paketi ekranı düşmüyordu — aynı işlem bir yerde hareket geçmişine giriyor, diğerinde girmiyordu. Talimat "aynı özelliği iki farklı yerde yeniden oluşturma" diyor (REVİZE 17) |
| `GV.hr.disKaynak(kod)` | İstihdam ilişkisi dış kaynak mı — `calismaTipi !== 'Kadrolu'` (alan yoksa eski vekil: `sozlesme === 'Hizmet sözleşmesi'`) | REVİZE 16 ekseni resmîleştirdi: `calismaTipi` **mesai** eksenli `calismaTuru`dan ayrı bir alandır (bir freelancer tam zamanlı da çalışabilir). Kural **tek yerdedir**, ekranlar kendi kuralını yazmaz |
...
> | `GV.destek.*` | `ops.js` (`tickets` · `ticketClosedStatuses`) |
> | `GV.proje.bakimPaketleri` · `GV.proje.bakimBagla` · `GV.proje.bakimAc` | `work.js` (`projects` · `activities`) · `crm.js` (`contracts`) · `ops.js` (`supportPackages` · `supportPackageTypes`) |
